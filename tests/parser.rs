...
fn parse_example_document() {
    ...
    let input =
        std::fs::read_to_string("examples/test.nutrition").expect("failed to read example file");
...
fn parse_ingredient_block_with_escaped_quotes() {
    let doc = parse_document(
        r#"@ingredient(100g)(1cup)(164g) "Chickpeas, \"Garbanzo\" Beans" {
  calories: 164kcal
  protein: 8.9g
  fat: 2.6g
  carbohydrates: 27.4g
...
fn parse_with_errors_returns_error_for_invalid_input() {
    ...
    assert!(
        !errors.is_empty(),
        "invalid input should report at least one error"
    );
}
...
@ingredient(100g) "good" {
    ...
    assert!(
        doc.is_some(),
        "should produce a partial document after recovery"
    );
    assert!(
        !errors.is_empty(),
        "should record an error for the malformed item"
    );
    let doc = doc.unwrap();
...
fn parse_with_errors_error_messages_include_line_number() {
    ...
    if !errors.is_empty() {
        ...
        assert!(
            has_line_ref,
            "error messages should reference a line number, got: {:?}",
            errors
        );
    }
}
...
fn diagnostics_empty_for_valid_source() {
    let (doc, diags) = parse_with_diagnostics(r#"@ingredient(100g) "chickpeas" { calories: 269 }"#);
    assert!(doc.is_some());
    assert!(
        diags.is_empty(),
...
fn diagnostics_carry_byte_span_into_source() {
    ...
    assert!(
        !diags.is_empty(),
        "malformed declaration should produce a diagnostic"
    );
    let d = &diags[0];
...
@ingredient(100g) "good" {
    ...
    assert!(
        ...
        "valid ingredient should be recovered"
    );
}
...
fn diagnostic_day_block_flags_unexpected_string() {
  ...
  "chickpeas"(1 cup)
    ...
    let msg = d
        .note_message
        .as_deref()
        .expect("note_message should be set");
    assert!(
...
fn diagnostic_day_block_note_span_within_source() {
  ...
  "chickpeas"(1 cup)
    ...
    if let Some(ref note_span) = d.note_span {
        ...
        assert!(note_span.end <= source.len(), "note_span.end out of bounds");
