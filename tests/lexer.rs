...
fn test_keywords() {
    let mut lexer =
        Token::lexer("@unit @property @ingredient @food @recipe @exercise @day @ate @exercised");
...
fn test_strings() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::String("test string".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_strings_with_escapes() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::String("tab\there".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::String("newline\n".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_identifiers() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("apple".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("banana".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("_private".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("name-with-dash".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("Test123".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_meal_labels() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::MealLabel("[Breakfast]".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::MealLabel("[Lunch]".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::MealLabel("[Morning Snack]".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_comments() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Comment("// comment".to_string())))
    );
...
fn test_newlines() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("cherry".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_whitespace_is_skipped() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("orange".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_identifier_with_numbers() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("var1".to_string())))
    );
    assert_eq!(
        lexer.next(),
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("_test123".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::Identifier("var-name".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_meal_label_with_special_chars() {
    ...
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::MealLabel("[Before Workout]".to_string())))
    );
    assert_eq!(
        lexer.next(),
        Some(Ok(Token::MealLabel("[Post-Workout Meal]".to_string())))
    );
    assert_eq!(lexer.next(), None);
}
...
fn test_comment_with_special_chars() {
    ...
    assert_eq!(
        ...
        Some(Ok(Token::Comment(
            "// this is a comment with !@#$%^&*() chars".to_string()
        )))
    );
    assert_eq!(lexer.next(), Some(Ok(Token::Newline)));
    assert_eq!(
